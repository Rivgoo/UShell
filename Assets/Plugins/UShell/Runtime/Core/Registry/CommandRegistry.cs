...
namespace UShell.Runtime.Core.Registry
{
	...
	{
		private readonly Dictionary<string, CommandSignature> _commands =
		...
		public bool TryGetCommand(string name, out CommandSignature signature) =>
			_commands.TryGetValue(name, out signature!);
		...
		private void RegisterKey(string key, CommandSignature sig)
		{
			if (_commands.ContainsKey(key))
				throw new ShellConfigurationException($"Command or alias '{key}' is already registered.");
			_commands[key] = sig;
